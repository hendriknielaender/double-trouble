...
|-|-|-|-|-|-|-|-|
|Github Stars|48.1k|15.3k|10.7k|4.6k|6.4k|18.1k|45.3k|
|Github Issues|5.5k|119|1.9k|296|383|660|1k|
|Bootstrap Time|18s|18s|61s|?|47s|?|?|
|First deployment time|25s|35s (why?)|70s|?|49s|?|?|
|Code deployment time|15s|15s|32s|?|31s|?|?|
|Dirty deployment time|15s|15s|12s|?|10s|?|?|
...
developer survey doesnt cover the IaC tool usage and studies on this are rare, it is re-assuring
to see that also such great engineers ponder on the question of tool popularity. In the podcast
they agreed that big enterprise companies mostly use terraform, while some positive points on the
serverless framework and the CDK were made. At the same time Yan Cui mentioned that the CDK is
less popular than he thought, which we can confirm. The positive points about the CDK made from
Stuvy dont seem substantial to us, as he also ends with "the real winner here is CloudFormation",
but the episode is still definitely worth a listen.

Stuvyenberg also points out that in some cases running bun lambdas from a container is more
beneficial than from a zip file. At the same time he does also recommend to run bun via a lambda
layer. [Chalice](https://aws.github.io/chalice/index.html) was also listed in their list of IaC
tools, which is yet to be added to our ranking.
