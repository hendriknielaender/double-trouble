...
<CodePreview ><div x-show="visible" className="-mt-6">
...
<div x-data="{ count: 0 }" class="flex flex-col">
  <span x-text="count" class="text-center"></span>
  <button className="rounded-sm px-2 bg-green-800" x-on:click="count++">Increment</button>
</div>
...
<div x-data="" class="flex w-full">
  <button className="bg-green-600 rounded-sm p-1 mx-auto"
    x-on:click="$refs.dialog.showModal();">open</button>
  <dialog x-ref="dialog"
    x-on:click="if (!$refs['dialog-inner'].contains($event.target)) {$refs.dialog.close();}"
    class="p-0 backdrop:bg-gray-400/50 mx-auto my-auto rounded-sm">
    <div x-ref="dialog-inner" class="p-4 space-y-4 flex flex-col">
      <span>This is an open dialog window</span>
      <button x-on:click="$refs.dialog.close();" class="bg-green-600 w-min rounded-sm">
        close
      </button>
    </div>
  </dialog>
...
<div class="relative" x-data="{selected: undefined, open: true, toggle() {this.open = !this.open}}">
